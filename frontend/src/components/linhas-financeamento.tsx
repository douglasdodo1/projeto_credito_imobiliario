...
export default function LinhasFinanceamento() {
  ...
  const tabelaCabecalhoClass = theme === "dark" ? "bg-zinc-800 text-zinc-100" : "bg-blue-200 text-blue-900";

  const linhaParClass = theme === "dark" ? "bg-gray-50 dark:bg-gray-800" : "bg-blue-50";

  const formContainerClass =
    theme === "dark"
      ? "p-6 bg-zinc-900 rounded-lg shadow-md border border-zinc-700"
      : "p-6 bg-white rounded-lg shadow-md border border-blue-200";

  const tituloFormClass =
    theme === "dark" ? "text-2xl font-semibold mb-4 text-indigo-400" : "text-2xl font-semibold mb-4 text-blue-700";
    ...
    <div className={cn("space-y-8 max-w-5xl mx-auto", theme === "dark" ? "text-white" : "text-gray-900")}>
      <h2
        className={cn(
          ...
          theme === "dark" ? "border-zinc-600" : "border-blue-600"
        )}
      >
      ...
      <div
        className={cn(
          ...
          theme === "dark" ? "border-zinc-700" : "border-blue-300"
        )}
      >
        <Table>
          <TableHeader className={tabelaCabecalhoClass}>
            <TableRow>
              <TableHead className={cn("py-3 px-6", theme === "dark" ? "text-zinc-100" : "text-blue-900")}>
                Tipo de Imóvel
              </TableHead>
              <TableHead className={cn("py-3 px-6", theme === "dark" ? "text-zinc-100" : "text-blue-900")}>
          ...
          <TableBody>
            {linhas.length === 0 ? (
              <TableRow>
                <TableCell
                  colSpan={2}
                  className={cn("text-center py-6", theme === "dark" ? "text-zinc-400" : "text-blue-600")}
                >
