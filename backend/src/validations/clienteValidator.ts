...
export class ClienteValidator {
  ...
  async verificarClienteUpdate(cliente: Partial<ClientEntradaDto>) {
    if (!this.isteste) {
      this.erros = [];

      if (cliente.cpf && !(await this.verificarCpfValido(cliente.cpf))) {
        this.erros.push(new Error(`CPF inválido: '${cliente.cpf}'`));
      }

      if (cliente.tipo && !(await this.validarTipo(cliente.tipo))) {
        this.erros.push(new Error(`Tipo inválido: '${cliente.tipo}'. Esperado: 'cliente' ou 'adm'`));
      }
